...
# CelebA-HQ 256px decoder-anchor preset without aux-4x4 context.
# Mirrors the provided hierarchy_hybrid_local_v2 command and saves checkpoints
# every 100 optimizer steps.

export MASTER_PORT="${MASTER_PORT:-8876}"
export CUDA_DEVICES="${CUDA_DEVICES:-4}"
...
export VAL_BATCH_SIZE_PER_GPU="${VAL_BATCH_SIZE_PER_GPU:-1}"
...
export HIERARCHY_STAGE_DEPTH="${HIERARCHY_STAGE_DEPTH:-2}"
...
export AUX_4X4_TARGET_RESOLUTION="${AUX_4X4_TARGET_RESOLUTION:-decoder_anchor}"
...
export NOTE="${NOTE:-decoder_anchor_noaux_ckpt100}"
export TIMESTAMP="${TIMESTAMP:-decoder_anchor_noaux_ckpt100}"

LEGACY_CKPT="${LEGACY_CKPT:-/SSD4/vipnu/hierarchical_zigma_v1/zigma/outputs/hierarchy_hybrid_local_v2_facehq_1024_bs32/2026-04-15_23-17-06_None}"
if [[ -z "${CKPT:-}" && -d "${LEGACY_CKPT}/checkpoints" ]]; then
  export CKPT="${LEGACY_CKPT}"
fi
