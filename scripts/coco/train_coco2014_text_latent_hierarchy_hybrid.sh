...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/train_coco2014_text_latent_hierarchy_hybrid.sh [--ckpt PATH] [--dry-run]
...
Common overrides:
  ...
  USE_CHECKPOINT=true
  USE_WANDB=true
  SAMPLE_FID_EVERY=100000
  SAMPLE_FID_N=5000
  NOTE=coco2014_raw_decoder_text_only_compress_type_router
...
NOTE="${NOTE:-coco2014_raw_decoder_text_only_compress_type_router}"
TIMESTAMP="${TIMESTAMP:-raw_decoder_text_only_compress_type_router}"
OPTIM_LR="${OPTIM_LR:-}"
EMA_RATE="${EMA_RATE:-}"
...
USE_INTEGRATED_ROUTER_CONTROLLER="${USE_INTEGRATED_ROUTER_CONTROLLER:-false}"
INTEGRATED_CONTROLLER_STAGE_TOP_K="${INTEGRATED_CONTROLLER_STAGE_TOP_K:-${STAGE_ROUTER_TOP_K}}"
INTEGRATED_CONTROLLER_HIDDEN_DIM="${INTEGRATED_CONTROLLER_HIDDEN_DIM:-auto}"
INTEGRATED_CONTROLLER_USE_CHANNEL_GATE="${INTEGRATED_CONTROLLER_USE_CHANNEL_GATE:-false}"
INTEGRATED_CONTROLLER_CHANNEL_GATE_SCALE="${INTEGRATED_CONTROLLER_CHANNEL_GATE_SCALE:-0.1}"
INTEGRATED_CONTROLLER_STAGE_SELECT_MODE="${INTEGRATED_CONTROLLER_STAGE_SELECT_MODE:-topk}"
INTEGRATED_CONTROLLER_STAGE_SELECT_THRESHOLD="${INTEGRATED_CONTROLLER_STAGE_SELECT_THRESHOLD:-0.5}"
INTEGRATED_CONTROLLER_STAGE_MIN_SELECTED="${INTEGRATED_CONTROLLER_STAGE_MIN_SELECTED:-1}"
...
if [[ "${USE_INTEGRATED_ROUTER_CONTROLLER}" == "true" && "${USE_MAMBA_SIZE_ROUTER}" == "true" ]]; then
  die "Integrated controller excludes the mamba-size router; set USE_MAMBA_SIZE_ROUTER=false."
fi
...
Launching COCO 2014 text-latent training
  ...
  integrated controller: ${USE_INTEGRATED_ROUTER_CONTROLLER}, mode=${INTEGRATED_CONTROLLER_STAGE_SELECT_MODE}, top_k=${INTEGRATED_CONTROLLER_STAGE_TOP_K}, min_selected=${INTEGRATED_CONTROLLER_STAGE_MIN_SELECTED}, threshold=${INTEGRATED_CONTROLLER_STAGE_SELECT_THRESHOLD}, channel_gate=${INTEGRATED_CONTROLLER_USE_CHANNEL_GATE}
